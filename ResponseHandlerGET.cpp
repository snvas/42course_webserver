...
void ResponseHandler::handlerGET(void) {
	...
	if(!_locationConf->default_file.empty() && isDirectory(getPath(sanitizedUri))){
		if (isFile(getPath(sanitizedUri + "/" + _locationConf->default_file))) {
			generateResponseFromFile(getPath(sanitizedUri + "/" + _locationConf->default_file));
		} else {
	...
	if (isDirectory(_conf.root + _req.uri)) {
		...
		generateResponseFromFile(getPath(_req.uri));
		_res.headers["Content-Type"] = getMimeType("." + getFileExtension(getPath(_req.uri)));
	} else {
		generateErrorResponse(404);
	}
}
...
void ResponseHandler::generate301RedirectResponse (std::string location) {
			...
			_res.body = "<html><head><title>301 Moved Permanently</title><script>window.location.replace('"
