...
void extractMultipartFormDataParts(const std::string& body, const std::string& bounday, std::vector<std::string>& parts){
	...
	while ((pos = body.find(delimiter, pos)) != std::string::npos){
		...
		std::string part = body.substr(pos + delimiter.size(), end - pos - delimiter.size());
...
bool createFileFromFormData(const std::string& part, const std::string& path){
	...
	if (headerEndPos == std::string::npos){
		std::cerr << "Could not find header boundary in form data part!" << std::endl;
		return false;
	}
...
void ResponseHandler::handlerPOST(void){
	...
	if (_req.headers["Content-Length"] != bodySizeStr){
		...
		std::cout << "Header Content-Length: " << _req.headers["Content-Length"] << std::endl;
	...
	if (isFile(path)){
		...
		std::cerr << "Exception occured: " + std::string(e.what()) << std::endl;
		generateErrorResponse(500);
	}
}}


void ResponseHandler::generateSucessResponse(int statusCode, const std::string &message)
{
	_res.statusCode = statusCode;
	_res.httpVersion = "HTTP/1.1";
	_res.headers["Content-Type"] = getMimeType(".html");
	std::ostringstream oss;
	oss << "<html><head><title>" << message << "</title></head><body><h1>" << message << "</h1></body></html>";
	_res.body = oss.str();
}
